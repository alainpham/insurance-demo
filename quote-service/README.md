...
This README provides instructions to build and run the quote-service using Docker.
...
To build the Docker image for the quote-service, run the following command:

```sh
docker rmi alainpham/quote-service
docker build -t alainpham/quote-service .
```

## Pushing to repository

```sh
docker push alainpham/quote-service
```
...
```sh
docker run --rm -p 8080:8080 --name quote-service alainpham/quote-service
```
