...
This README provides instructions to build and run the workflow-service using Docker.
...
export OTEL_SERVICE_NAME="workflow-service"
...
To build the Docker image for the workflow-service, run the following command:

```sh
docker rmi alainpham/workflow-service
docker build -t alainpham/workflow-service .
```

## Pushing to repository

```sh
docker push alainpham/workflow-service
```
...
```sh
docker run --rm -p 8080:8080 --name workflow-service alainpham/workflow-service
```
